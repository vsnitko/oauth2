...
public class UserServiceImpl implements UserService, UserDetailsService, OAuth2AuthorizedClientService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        return userRepository.findByEmail(email)
            ...
            .orElseThrow(() -> new UsernameNotFoundException(String.format("User with %s email does not exist",
                                                                           email)));
    }

    @Override
    public void saveAuthorizedClient(OAuth2AuthorizedClient authorizedClient, Authentication principal) {
        final OAuth2AuthenticationToken tokenPrincipal = (OAuth2AuthenticationToken) principal;
        final User builtUser = userBuilder.buildUser(authorizedClient, tokenPrincipal);
        final User user = userRepository.findByEmail(builtUser.getEmail())
